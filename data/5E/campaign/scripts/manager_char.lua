...
function onAbilitySelectDialog(nodeChar, aAbilities, nPicks, nAbilityAdj, nAbilityMax, bSaveProfAdd)
	...
	local rAbilityAdd = { nodeChar = nodeChar, nAbilityAdj = nAbilityAdj, nAbilityMax = nAbilityMax, bSaveProfAdd = bSaveProfAdd };
...
function onAbilitySelectComplete(aSelection, rAbilityAdd)
	for _,sAbility in ipairs(aSelection) do
		addAbilityAdjustment(rAbilityAdd.nodeChar, sAbility, rAbilityAdd.nAbilityAdj or 1, rAbilityAdd.nAbilityMax);
	...
	if sTraitType == "abilityscoreincrease" then
		...
		if sAdjust:match("your ability scores each increase") then
			...
			if sIncrease then
				...
				onAbilitySelectDialog(nodeChar, nil, 2, nAbilityAdj);
				bApplied = true;
			else
				sIncrease = sAdjust:match("one ability score of your choice increases by (%d+)");
				if sIncrease then
					local nAbilityAdj = tonumber(sIncrease) or 1;
					onAbilitySelectDialog(nodeChar, nil, 1, nAbilityAdj);
					bApplied = true;
				else
					...
					if sIncrease then
						...
						if #aAbilities > 0 then
							local nAbilityAdj = tonumber(sIncrease) or 1;
							onAbilitySelectDialog(nodeChar, aAbilities, 1, nAbilityAdj);
							bApplied = true;
						end
					else
						...
						if sIncrease then
							...
							if #aAbilities > 0 then
								local nAbilityAdj = tonumber(sIncrease) or 1;
								onAbilitySelectDialog(nodeChar, aAbilities, 2, nAbilityAdj);
								bApplied = true;
							end
						else
...
function checkFeatAdjustments(nodeChar, sText)
	...
	if sAbility then
		...
		if sAbility1 and sAbility2 then
			local nAbilityAdj = tonumber(nAdj) or 1;
			local nAbilityMax = tonumber(sAbilityMax) or 20;
			onAbilitySelectDialog(nodeChar, { sAbility1, sAbility2 }, 1, nAbilityAdj, nAbilityMax);
		else
			...
			if sAbility1 and sAbility2 and sAbility3 then
				local nAbilityAdj = tonumber(nAdj) or 1;
				local nAbilityMax = tonumber(sAbilityMax) or 20;
				onAbilitySelectDialog(nodeChar, { sAbility1, sAbility2, sAbility3 }, 1, nAbilityAdj, nAbilityMax);
			else
				-- XGtE - Elven Accuracy
				...
				if sAbility1 and sAbility2 and sAbility3 and sAbility4 then
					local nAbilityAdj = tonumber(nAdj) or 1;
					local nAbilityMax = tonumber(sAbilityMax) or 20;
					onAbilitySelectDialog(nodeChar, { sAbility1, sAbility2, sAbility3, sAbility4 }, 1, nAbilityAdj, nAbilityMax);
				else
					-- PHB - Resilient
					...
					if nAdj and nAbilityMax then
						local nAbilityAdj = tonumber(nAdj) or 1;
						local nAbilityMax = tonumber(sAbilityMax) or 20;
						onAbilitySelectDialog(nodeChar, nil, 1, nAbilityAdj, nAbilityMax, true);
					end
				end
			end
		end
